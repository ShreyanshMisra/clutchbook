...
> Companion document: [`roadmap.md`](./roadmap.md) — what we build, in what order. Read this first for product definition.
>
> The pre-pivot docs (sportsbook framing, house-banked "Clutchbook" contracts, corp/data notes) are archived under [`docs/old/`](./old/) for reference. They are deprecated.
...
Because money match is peer-to-peer, the engine's job is **pairing**, not pricing. The matchmaking service keys on `(game, format, entry tier, rating band)` and widens the rating tolerance over time until a fair match is found. The former "odds engine" is repurposed into a **skill-rating + bracketing service**: it estimates each player's strength to bracket them and to surface an honest "you're well-matched / this is a reach" signal — it does **not** set a payout line. (Implementation home: `api/_lib/odds_engine.py`, to be renamed.)
...
- `api/_lib/odds_engine.py` → skill-rating + bracketing service (no longer prices odds).
